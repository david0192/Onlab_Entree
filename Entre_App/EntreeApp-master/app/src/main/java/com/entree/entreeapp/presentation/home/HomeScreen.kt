...
@Composable
fun HomeScreen(navcontroller: NavHostController, boughtTicketTypeId:IntWrapper){
    Scaffold(bottomBar = {
        BottomBar(navController=navcontroller, roleId = Roles.GUEST.value)
    }) {
        SetUpNavGraph(navController = navcontroller, boughtTicketTypeId = boughtTicketTypeId)
    }
}
...
fun AdminHomeScreen(navcontroller: NavHostController){
    Scaffold(bottomBar = {
        ...
        AdminSetUpNavGraph(navController = navcontroller)
    }
}
